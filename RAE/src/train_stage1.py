...
class ImageNetTarDataset(IterableDataset):
    ...
    def __len__(self) -> int:
        if self.dataset_size is None:
            raise TypeError("Dataset length is undefined for streaming WebDataset shards.")
    ...
    def _iter_shard(self, shard_path: Path, rng: random.Random):
        with tarfile.open(shard_path, "r") as tar:
            ...
            for sample_id in sample_ids:
                image_member = members.get(f"{sample_id}.jpg")
                cls_member = members.get(f"{sample_id}.cls")
                if image_member is None or cls_member is None:
                    continue
                image_file = tar.extractfile(image_member)
                label_file = tar.extractfile(cls_member)
    ...
    def __iter__(self):
        ...
        shard_indices = shard_indices[self.rank :: self.world_size]
        shard_indices = shard_indices[worker_id :: num_workers]
        for shard_idx in shard_indices:
            shard_path = self.shard_paths[shard_idx]
            shard_rng = random.Random(self.base_seed + self._epoch * 7919 + shard_idx)
            yield from self._iter_shard(shard_path, shard_rng)
...
def prepare_dataloader(
...
) -> Tuple[DataLoader, DistributedSampler | ImageNetTarDataset]:
    ...
    shard_paths = sorted(data_path.glob("*.tar"))
    if shard_paths:
        if dataset_num_samples is None:
            raise ValueError(
                "dataset_num_samples must be provided in the config when using WebDataset tar shards."
            )
        dataset = ImageNetTarDataset(
            ...
            base_seed=base_seed,
            dataset_size=int(dataset_num_samples),
        )
        loader = DataLoader(
            dataset,
...
def main():
    ...
    dataset_num_samples_cfg = training_cfg.get("dataset_num_samples", None)
    dataset_num_samples = int(dataset_num_samples_cfg) if dataset_num_samples_cfg is not None else None
    ...
    loader, epoch_controller = prepare_dataloader(
        args.data_path,
        args.image_size,
        batch_size,
        num_workers,
    ...
    try:
        ...
        if dataset_num_samples is None:
            raise RuntimeError("Iterable dataset requires training.dataset_num_samples to compute steps per epoch.")
        steps_per_epoch = dataset_num_samples // (batch_size * world_size)
    ...
    if rank == 0:
        ...
        try:
            per_process_samples = len(loader.dataset)
        except TypeError:
            per_process_samples = None
        if per_process_samples is not None:
            if isinstance(epoch_controller, ImageNetTarDataset):
                dataset_sample_count = per_process_samples * world_size
            else:
                dataset_sample_count = per_process_samples
        else:
            dataset_sample_count = dataset_num_samples
        dataset_msg = dataset_sample_count if dataset_sample_count is not None else "unknown"
        logger.info(f"Dataset contains {dataset_msg} samples, {steps_per_epoch} steps per epoch.")
